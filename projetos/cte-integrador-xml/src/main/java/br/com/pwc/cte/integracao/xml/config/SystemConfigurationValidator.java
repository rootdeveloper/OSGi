...
public class SystemConfigurationValidator {
	...
	private void validarArquivo(String pathFile) {
		...
		if (file.exists())
			throw new IllegalArgumentException("O arquivo não existe " + pathFile);
		if (file.isFile())
			throw new IllegalArgumentException("Não é um arquivo" + pathFile);
	}
	...
	private static final String DIR_CONFIG_SYSTEM = "config/xmlCTe.properties";
